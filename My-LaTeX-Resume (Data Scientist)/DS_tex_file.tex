...
\documentclass[letterpaper]{DS_class_file} % a4paper for A4
	...
	\programming{{$\textbullet$ AWS $\textbullet$ Airflow $\textbullet$ Kubernetes / 2.0}, 
	{$\textbullet$ Microsoft Office $\textbullet$ MLOps $\textbullet$ Git / 3.0}, 
	{$\textbullet$ Linux $\textbullet$ SQL $\textbullet$ Docker  / 4.0},
	{$\textbullet$ HTML $\textbullet$ CSS $\textbullet$ JavaScript / 5.0},
	{$\textbullet$ Python $\textbullet$ Java / 6.0}}
	\vspace{10mm}
	
	
	% \programminglanguage{{ Arabic (A2 - Elementary Level) / 2.5 }, { English (C1 - Professional Work Ability) / 5}, { Turkish (C2 - Native Language) / 6}}
	
	% Command for printing technical skills 
	\technicalskills{
		{$\textbullet$ Kafka} / 4.0,
		{$\textbullet$ ELK (Elasticsearch, Logstash, Kibana)} / 3.5,
		{$\textbullet$ Big Data Tools (e.g., Spark, Hadoop)} / 4.0,
		{$\textbullet$ IBM Cloud (Watson Studio)} / 4.5,
		{$\textbullet$ Database Management System} / 5.0,
		{$\textbullet$ Machine Learning} / 5.0,
		{$\textbullet$ Data Analytics, Data Visualization} / 6.0
	}
	...
	\cvname{\hspace{-0.5cm}Achhuta Nand\newline\newline} % My name.
	\cvsurname{\hspace{-0.6cm}Jha} % My surname
	\cvjobtitle{\hspace{0.05cm}Data Scientist} % My position.
	% title/career
	
	\cvlinkedin{Achhuta Nand Jha}
	\cvgithub{anjha1}
	\cvnumberphone{+90 7281031975} % Phone number
	\cvsite{India-Mathura} % Where I live.
	\cvwebsite{anjha1}
	\cvmail{achhuta.jha_cs.da23@gla.ac.in} % Email address
